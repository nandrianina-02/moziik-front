...
const AccountView = ({ token, userNom, userEmail, userRole, userId: userIdProp, userArtistId: userArtistIdProp, isAdmin, isArtist, isUser, musiques, userPlaylists, onUpdateProfile }) => {
  // Fallback to localStorage if props are undefined (happens on page refresh before verify completes)
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      if (trimmedNom && trimmedNom !== userNom) {
        let endpoint = '';
        if (isArtist && userArtistId) {
          endpoint = `${API}/artists/${userArtistId}`;
        } else if (userId) {
        ...
        if (!endpoint) {
          throw new Error('ID de profil introuvable — veuillez vous reconnecter.');
        }
        ...
        const res = await fetch(endpoint, {
          ...
          body: JSON.stringify({ nom: trimmedNom })
        });
        ...
        localStorage.setItem('moozik_nom', trimmedNom);
        if (onUpdateProfile) onUpdateProfile({ nom: trimmedNom });
      }

      setSuccessProfile('Profil mis à jour avec succès !');
      setTimeout(() => setSuccessProfile(''), 4000);
    } catch (err) {
      setErrorProfile(err.message || 'Une erreur est survenue');
    } finally {
  ...
  const handleChangePassword = async (e) => {
    ...
    setErrorPassword('');
    setSuccessPassword('');
    if (!currentPassword) return setErrorPassword('Veuillez saisir votre mot de passe actuel');
    if (!newPassword || newPassword.length < 6) return setErrorPassword('Le nouveau mot de passe doit faire au moins 6 caractères');
    if (currentPassword === newPassword) return setErrorPassword('Le nouveau mot de passe doit être différent');

    setLoadingPassword(true);
    try {
      let endpoint = '';
      if (isArtist && userArtistId) {
        endpoint = `${API}/artists/${userArtistId}/password`;
      } else if (userId) {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        if (res.status === 404) throw new Error('Cette fonctionnalité n\'est pas disponible sur ce serveur');
      ...
      setSuccessPassword('Mot de passe changé avec succès !');
      setCurrentPassword('');
      setNewPassword('');
      setTimeout(() => setSuccessPassword(''), 4000);
    } catch (err) {
      setErrorPassword(err.message || 'Une erreur est survenue');
    } finally {
  ...
  const profileId = isArtist ? userArtistId : userId;
    ...
    <div className="animate-in fade-in duration-500 max-w-2xl mx-auto space-y-5">
        ...
        {displayAvatar && (
          <div className="flex items-center gap-3 mb-5 p-3 bg-zinc-800/60 rounded-xl">
            ...
            <button type="button" onClick={removeAvatar}
              className="text-zinc-500 hover:text-red-500 p-1.5 hover:bg-red-500/10 rounded-lg transition">
              <Trash2 size={13} />
            </button>
          </div>
        )}
        ...
        <form onSubmit={handleSaveProfile} className="flex flex-col gap-4">
          <div>
            ...
            <input value={nom} onChange={e => setNom(e.target.value)} placeholder="Votre prénom ou pseudo"
              disabled={!profileId && !isAdmin}
              className="w-full bg-zinc-800 rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 ring-red-600 text-white placeholder-zinc-600 transition disabled:opacity-40 disabled:cursor-not-allowed" />
          </div>
          <div>
          ...
          {errorProfile && (
            <div className="flex items-start gap-2 bg-red-500/10 border border-red-500/20 text-red-400 text-xs px-4 py-3 rounded-xl">
              <AlertCircle size={14} className="shrink-0 mt-0.5" /><span>{errorProfile}</span>
            </div>
          )}
          {successProfile && (
            <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 text-green-400 text-xs px-4 py-3 rounded-xl">
              <CheckCircle size={14} /> {successProfile}
            </div>
          )}
      ...
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-6 md:p-8">
        ...
        <form onSubmit={handleChangePassword} className="flex flex-col gap-4">
          ...
          <div>
            ...
            {newPassword && (
              <div className="mt-2 flex items-center gap-1.5">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= pwdStrength ? pwdColors[pwdStrength] : 'bg-zinc-700'}`} />
                ))}
          ...
          {errorPassword && (
            <div className="flex items-start gap-2 bg-red-500/10 border border-red-500/20 text-red-400 text-xs px-4 py-3 rounded-xl">
              <AlertCircle size={14} className="shrink-0 mt-0.5" /><span>{errorPassword}</span>
            </div>
          )}
          {successPassword && (
            <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 text-green-400 text-xs px-4 py-3 rounded-xl">
              <CheckCircle size={14} /> {successPassword}
            </div>
          )}
